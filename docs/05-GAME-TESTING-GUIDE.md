...
ATP Platform включает 8 канонических игр в пакете `game-environments`:
...
| **Бар Эль-Фарола** | Дискретный (go/stay) | 2-100 | Игра меньшинства, ограниченная рациональность, предсказание посещаемости |
...
### 6.4. Бар Эль-Фарола — предсказание посещаемости
...
game:
  ...
  config:
    num_players: 20
    num_rounds: 100
    capacity_threshold: 0.6   # Бар приятен, если <= 60% приходят

episodes: 20

baselines:
  - "random"
  - "threshold_history"       # Идти, если прошлая посещаемость < порога
...
**Ожидания:** хороший агент учится предсказывать посещаемость и ходит в бар только когда мало других посетителей (динамика игры меньшинства).
